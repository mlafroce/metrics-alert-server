...
use tp1::metric::{MetricAction, QueryAggregation, QueryParams, QueryParamsSerializable};
...
fn run_queries(metric_path: &str, host_addr: &str) -> io::Result<()> {
    ...
    let queries = reader
        .lines()
        .flatten()
        .flat_map(|line| serde_json::from_str::<QueryParamsSerializable>(&line))
        .map(|qp| qp.to_query_params())
